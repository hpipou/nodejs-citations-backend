...
const newPassword = (req,res,next)=>{
    ...
    }else if(req.body.newpassword==null || req.body.newpassword==undefined){
        return res.status(403).json("NEW PASSWORD UNDEFINDED")
    }
    ...
    if(validator.isEmpty(newpassword)){return res.status(403).json("NEW PASSWORD UNDEFINDED")}
    ...
    if(!validator.isLength(newpassword,{min:5, max:30})){return res.status(403).json("NEW PASSWORD MIN:5 MAX:30")}
