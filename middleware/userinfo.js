...
const userVerif = (req,res,next)=>{
    ...
    if(req.body.email==null || req.body.email==undefined){
        ...
        return res.status(403).json("USERNAME UNDEFINDED")
    ...
    if(validator.isEmpty(username)){return res.status(403).json("USERNAME UNDEFINDED")}
    ...
    if(!validator.isLength(username,{min:5, max:30})){return res.status(403).json("USERNAME MIN:5 MAX:30")}
